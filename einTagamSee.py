...
V = np.zeros((T + 1, 5, 5))  # Maximum 8 successful actions to track, safe upper bound
                            # Limit is set to 8 is because each action sw and sb need to be successfully done at least 2 times (11 - 2 = 9)
                            # During the day, action eat is done 1 time (9 - 1 =8)
...
# Backward DP
policy = np.empty((T, 5, 5), dtype=object)
...
print('Policy: ', policy)
Utils.print_optimal_day(policy, V, T)
